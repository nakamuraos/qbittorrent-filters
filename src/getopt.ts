...
function getShorteners(options: Config): { [key: string]: string } {
  ...
  return Object.entries(options).reduce((accum, [key, value]: any) => {
...
function getStateAndReset(state: ParsingState): {
  [key: string]: Array<string | boolean>;
} {
...
function checkRequiredParams(
  config: Config,
  input: GetoptPartialResponse
): void {
  if (config._meta_ && typeof config._meta_ === "object") {
    const { args = 0, minArgs = 0, maxArgs = 0 } = config._meta_ as any;

    let providedArgs = 0;
    let error: any = null;
    if (
      Array.isArray(input[POSITIONAL_ARGS_KEY]) &&
      input[POSITIONAL_ARGS_KEY].length > 0
    ) {
  ...
  Object.entries(config).forEach(([key, value]: any) => {
    ...
    if (value.args && value.args !== "*") {
      ...
      if (expectedArgsCount > 0 && expectedArgsCount !== argsCount) {
        throwError(
          `Option "--${key}" requires ${expectedArgsCount} arguments, but ${argsCount} were provided`
        );
      }
    }
  });
...
function applyDefaults(config: Config, result: GetoptPartialResponse): void {
  Object.entries(config).forEach(([key, value]) => {
    ...
    if ("default" in value && !(key in result)) {
      const values = Array.isArray(value.default)
        ? value.default
        : [value.default];
...
function getopt(config: Config = {}, command: string[]): GetoptResponse {
  ...
  rawArgs.forEach((arg) => {
    ...
    parsedOption.forEach((option) => {
      if (["h", "help"].includes(option)) throwError("help");

      let subconfig: any = config[option];
      if (!subconfig) {
...
function getHelpMessage(config: Config, programName: string): string {
  ...
  const maxLength = lines.reduce(
    (prev, current) => Math.max(current[0].length, prev),
    0
  );
...
function checkConfig(config: Config): void {
  if (config.help)
    throw new Error(
      '"--help" option is reserved and cannot be declared in a getopt() call'
    );
  Object.values(config).forEach((value: any) => {
    ...
    if (value.key === "h")
      throw new Error(
        '"-h" option is reserved and cannot be declared in a getopt() call'
      );
    if (value.mandatory)
      console.warn(
...
export default (
  ...
  try {
    ...
    const message = (
      error.message.replace(ERROR_PREFFIX, "") +
      "\n" +
      getHelpMessage(config, programName)
    ).trim();
    if (printOnFailure) console.warn(message);
    if (exitOnFailure)
      process.exit(error.message === ERROR_PREFFIX + "help" ? 0 : FAILURE);
